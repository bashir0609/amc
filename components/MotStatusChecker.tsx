...
export default function MotStatusChecker() {
      ...
      {motData && (
        <div className="container-custom py-16">
          <div className="max-w-5xl mx-auto space-y-8">
            {/* Trap Door Top CTA */}
            <div className="text-center bg-white p-6 rounded-2xl border-2 border-primary-100 shadow-lg mb-10 flex flex-col md:flex-row items-center justify-between gap-6 text-gray-900">
              <div className="text-left">
                ...
                <p className="text-gray-600">
                  Secure your slot today. Walk-ins welcome, but booking avoids the wait.
                </p>
              </div>
              <Link
                href={`/mot-booking?vrm=${motData.registration}`}
                className="btn-primary whitespace-nowrap px-8 py-4 text-lg shadow-xl shrink-0"
              >
                Book Your MOT Now
              </Link>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden text-gray-900">
              ...
              <div className="p-6 grid grid-cols-2 md:grid-cols-4 gap-6">
                <div>
                  ...
                  <p className="font-semibold text-lg">{toTitleCase(motData.make)}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500 mb-1">Model</p>
                  <p className="font-semibold text-lg">{toTitleCase(motData.model)}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500 mb-1">Colour</p>
                  <p className="font-semibold text-lg">{toTitleCase(motData.primaryColour)}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500 mb-1">Fuel Type</p>
                  <p className="font-semibold text-lg">{toTitleCase(motData.fuelType)}</p>
                </div>
                {motData.manufactureDate && (
                  <div>
                    <p className="text-sm text-gray-500 mb-1">Year of Manufacture</p>
                    <p className="font-semibold text-lg">{motData.manufactureDate.split(".")[0]}</p>
                  </div>
                )}
              </div>
            </div>
                    ...
                    {motData.motTests.map((test, idx) => {
                      ...
                      return (
                        <div key={idx} className="relative pl-8">
                          {/* Timeline dot */}
                          <div
                            className={`absolute -left-3.5 top-1.5 w-7 h-7 rounded-full border-4 border-white flex items-center justify-center ${
                          ...
                          <div
                            className={`p-5 rounded-xl border ${
                              isPass ? "border-green-100 bg-green-50/30" : "border-red-100 bg-red-50/30"
                            }`}
                          >
                            <div className="flex flex-col md:flex-row md:items-start justify-between mb-4 gap-4">
                              <div>
                                <span
                                  className={`inline-flex px-3 py-1 rounded-full text-sm font-bold mb-3 ${
            ...
            <div className="text-center bg-gray-900 text-white p-10 rounded-2xl shadow-xl mt-10 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <h2 className="text-3xl font-heading font-bold mb-4">
                  Ready to test your {toTitleCase(motData?.make)}?
                </h2>
                <p className="text-gray-300 text-lg mb-8 max-w-2xl mx-auto">
                  Whether you need a full service, repairs, or just your annual MOT, Auto MOT Centre
                  guarantees DVSA certified, honest work.
                </p>
                <Link
                  href={`/mot-booking?vrm=${motData.registration}`}
                  className="btn-primary text-xl px-10 py-5 shadow-2xl hover:scale-105 transform transition-all duration-300 inline-block"
                >
